...
Demonstrate how Shiny apps, widgets and components can be integrated with .Rmd documents ( .html, ioslides)

## Topics covered:

-   Demonstration of inline apps: shinyApp() function

-   Demonstration of embedding full apps: using app URL

    For more: \<[[https://bookdown.org/yihui/rmarkdown/shiny-documents.html\\\\](https://bookdown.org/yihui/rmarkdown/shiny-documents.html\\){.uri}]([https://bookdown.org/yihui/rmarkdown/shiny-documents.html\\](https://bookdown.org/yihui/rmarkdown/shiny-documents.html\){.uri}){.uri}\>

-   Interactive mapping with leaflet

    -   adding grouped and custom markers
...
-   leaflet: choose a basemap: [<https://leaflet-extras.github.io/leaflet-providers/preview/>](https://leaflet-extras.github.io/leaflet-providers/preview/){.uri}
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  ui = fluidPage(
         fluidRow(
           column(
             width = 12,  
                  ...
                  options = list(`style` = "btn-danger")# end choices
                  )#end pickerinput
               )#end column
           ), # end fluidrow
         ...
         fluidRow(
               ...
               # leaflet map output
                 withSpinner(
                    leafletOutput("map",height = "420px", width="100%")#end leaflet
                    )#end spinner
                   ),#end column
                )#end fluid row
  ), # endFluidPage
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    output$map<-renderLeaflet({
        leaflet(sa2_merged) %>%
            #addProviderTiles(leaflet::providers$Esri.WorldGrayCanvas) %>%
            #addProviderTiles(leaflet::providers$Esri.WorldImagery) %>%
            addProviderTiles(leaflet::providers$Esri.WorldStreetMap ) %>%
...
# define groups for markers based on categorical variable
    ...
    observe({ 
        if (input$stats_input == "SA2_NAME16") {
            pal <- colorFactor(mycol(30), #mycol is a colourRamp pal we set at beginning
                               domain= sa2_merged[[input$stats_input]])
        } else {
            pal <- colorNumeric(c("#008B00","#C12525"), 
                                domain = sa2_merged[[input$stats_input]], 
...
## Clustering markers \| reduces clutter
