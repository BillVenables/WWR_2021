...
only have \Sexpr{english(with(CC, sum(!is.na(credit.card.owner))))}
\emph{at most} from \Sexpr{english(nrow(CC))} cases.
...
dim(CCKnown)
@
For model building we have just \Sexpr{english(nrow(CCKnown))} cases and
\Sexpr{english(ncol(CCKnown)-1)} potential predictors.
...
completeness.  (The coding details are not important for our primary
purpose here.)
<<oneSE>>=
WWRCourse::oneSERule
methods("oneSERule")
...
rm(n, X)
@
So in this simulation the cases were sampled between \Sexpr{english(lims[1])}
and \Sexpr{english(lims[2])} times.  This seems about enough.
